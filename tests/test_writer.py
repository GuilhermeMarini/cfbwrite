...
# The directory/red-black assertions are white-box: they check the
# format's own invariants against the writer's private constants.
from cfbwrite import _writer as _w
...
def test_a_large_stream_roundtrips_through_the_main_fat(tmp_path):
    ...
    dst = tmp_path / "out.bin"
    cfb.write_ole(dst, [_stream("Big", data)])
    ole = olefile.OleFileIO(str(dst))
    try:
...
def test_a_stream_exactly_at_the_cutoff_is_not_mini(tmp_path):
    data = b"x" * 4096
    dst = tmp_path / "out.bin"
    cfb.write_ole(dst, [_stream("Edge", data)])
    ole = olefile.OleFileIO(str(dst))
    try:
...
def test_an_empty_stream_roundtrips(tmp_path):
    dst = tmp_path / "out.bin"
    cfb.write_ole(dst, [_stream("Empty", b"")])
    ole = olefile.OleFileIO(str(dst))
    try:
...
def test_many_siblings_are_all_reachable(tmp_path):
    ...
    cfb.write_ole(dst, [_stream(n, n.encode()) for n in names])
    ole = olefile.OleFileIO(str(dst))
    try:
        for n in names:
...
def test_a_name_too_long_is_refused(tmp_path):
    with pytest.raises(cfb.CfbWriteError):
        cfb.write_ole(tmp_path / "out.bin", [_stream("x" * 32, b"a")])
...
def test_rebuild_replaces_a_stream_with_a_longer_one(tmp_path):
    ...
    cfb.rebuild(src, dst, {("Relays", "R1", "SET_D1.TXT"): grown})
...
def test_rebuild_refuses_a_replacement_for_a_stream_that_is_not_there(tmp_path):
    src = tmp_path / "src.bin"
    cfb.write_ole(src, [_stream("A", b"a")])
    with pytest.raises(cfb.CfbWriteError):
        cfb.rebuild(src, tmp_path / "dst.bin", {("Nope",): b"x"})


def test_rebuild_deletes_the_output_when_verification_fails(tmp_path, monkeypatch):
    src = tmp_path / "src.bin"
    cfb.write_ole(src, [_stream("A", b"aaaa")])
    dst = tmp_path / "dst.bin"
    ...
    monkeypatch.setattr(_w, "write_ole", sabotaged)
    with pytest.raises(cfb.CfbWriteError):
        cfb.rebuild(src, dst, {})
...
def test_a_stream_big_enough_to_need_a_difat_chain_roundtrips(tmp_path):
    ...
    dst = tmp_path / "out.bin"
    cfb.write_ole(dst, [_stream("Huge", data)])
...
def _assert_red_black(entries, root_sid):
    ...
    if root_sid == cfb.NOSTREAM:
        return
    assert entries[root_sid]["colour"] == _w._COLOR_BLACK, "red root"
    ...
    def walk(sid, blacks):
        if sid == cfb.NOSTREAM:
            ...
            return
        e = entries[sid]
        if e["colour"] == _w._COLOR_RED:
            for kid in (e["left"], e["right"]):
                assert (kid == cfb.NOSTREAM
                        or entries[kid]["colour"] == _w._COLOR_BLACK), \
                    f"red-red edge under {e['name']!r}"
        deeper = blacks + (1 if e["colour"] == _w._COLOR_BLACK else 0)
        walk(e["left"], deeper)
        order.append(e["name"])
        walk(e["right"], deeper)
...
def test_a_failed_write_leaves_the_destination_untouched(tmp_path, monkeypatch):
    ...
    def exploding(path, children):
        ...
        raise cfb.CfbWriteError("blew up halfway through the write")

    monkeypatch.setattr(_w, "write_ole", exploding)
    with pytest.raises(cfb.CfbWriteError):
        cfb.rebuild(src, dst, {})
...
def test_rebuilding_onto_the_source_is_refused(tmp_path):
    ...
    cfb.write_ole(path, [_stream("A", b"aaaa"),
                         _stream("B", b"b" * 9000)])
    before = path.read_bytes()

    with pytest.raises(cfb.CfbWriteError):
        cfb.rebuild(path, path, {})
    assert path.read_bytes() == before

    with pytest.raises(cfb.CfbWriteError):
        cfb.rebuild(path, tmp_path / "." / "same.bin", {})
    assert path.read_bytes() == before
