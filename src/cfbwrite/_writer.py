"""Write a complete Compound File (MS-CFB v3), so a stream can change size.

``olefile`` replaces a stream only with data of exactly the same length. That
is enough to rearrange existing content -- moving bytes around conserves them
-- and not enough to add any, so anything that GROWS a stream has no slack to
...
that were meant to change, and it writes atomically. That is not caution for
its own sake -- this came out of a tool that rewrites protective relay settings
files, where a bug in the writer has to surface as a failed write and never as
...
def write_ole(dst: Path, root_children: list[Entry]) -> None:
    ...
    for n in nodes:
        if len(n.name) > 31:
            raise CfbWriteError(
                f"name too long for the CFB format (max 31): {n.name!r}")
    ...
    for _ in range(16):                      # converges in 2-3 rounds
        ...
        raise CfbWriteError("could not size the FAT")
    ...
    with dst.open("wb") as fh:
        ...
        for n in reg_nodes:
            ...
            if len(data) != n.size:
                raise CfbWriteError(
                    f"{n.name}: declared size {n.size}, read {len(data)}")
...
def _write_directory(fh, dir_bytes: bytes, sectors: int) -> None:
    ...
    if slack < 0 or slack % DIR_ENTRY_SIZE:
        raise CfbWriteError("the directory does not fit the sectors reserved for it")
    fh.write(dir_bytes)
...
def _write_padded(fh, data: bytes, sectors: int) -> None:
    ...
    if pad < 0:
        raise CfbWriteError("data larger than the sectors reserved for it")
    if pad:
        fh.write(b"\x00" * pad)
...
def _dir_entry(name: str, etype: int, left: int, right: int, colour: int,
    ...
    if len(encoded) > 64:
        raise CfbWriteError(f"name too long for the CFB format: {name!r}")
    return b"".join((
...
def rebuild(src: Path, dst: Path,
    ...
    the temporary is removed and ``CfbWriteError`` is raised: a half-right
    RDB is worse than none.
    """
    src, dst = Path(src), Path(dst)
    if _is_same_file(src, dst):
        raise CfbWriteError(
            "cannot rebuild onto the source file itself: "
            f"{src}")
    ...
    try:
        ...
        try:
            ...
            if missing:
                raise CfbWriteError(
                    "stream not present in the source: "
...
def _verify(src: Path, dst: Path,
    ...
    try:
        ...
        if ea != eb:
            raise CfbWriteError(
                "the rebuilt tree does not match the source tree")
        for raw in a.listdir(streams=True, storages=False):
            ...
            if got != want:
                raise CfbWriteError(
                    f"stream differs after rebuild: {'/'.join(path)}")
    finally:
        a.close()
        b.close()
